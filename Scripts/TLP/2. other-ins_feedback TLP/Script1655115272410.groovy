...
token = WS.sendRequest(findTestObject('TLP/get_credential TLP'))
...
feedback = WS.sendRequest(findTestObject('TLP/OtherInsFeedbackTLP', [('token') : private_token, ('id') : id
            , ('maskapai_id') : maskapai_id, ('source') : source, ('orderdate') : orderdate, ('no_reff') : accno, ('no_polis') : no_polis]))

WS.verifyElementPropertyValue(feedback, 'SubmitPolisAsuransiTLCResponse.StatusCode', '00', FailureHandling.CONTINUE_ON_FAILURE)

'show response to report in test suite'
result_feedback = feedback.getResponseText()

KeywordUtil.markPassed('order date '+ orderdate + ' data ke : ' + no)
